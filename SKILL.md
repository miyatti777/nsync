...
> ⚠️ **スコープは「ワークスペースroot全体」（サブフォルダ単位ではない）:** `sync` は `.nsync.yaml` の `root_page_id` 配下 **全体** を対象にする（コマンドにサブフォルダ引数はない）。深いサブフォルダで作業していても `sync` すると root 配下すべてのローカル変更がPush・全リモート変更がPullされる。**特定フォルダだけを独立同期したいなら、そのNotionページを `root` にした専用ワークスペースを別途 `init` する**。単発のサブツリー操作なら `pull -r <url>` / `push -r <file>` を使う。「今のrootがどこか」は作業前に `.nsync.yaml` の `root_page_id`・`label` で確認する。

> ⚠️ **`sync` は機密も無差別にPushする:** ローカルの全変更を自動Pushするため、Notionに出したくないファイル（機密メモ等）があれば ①`.nsync.yaml` の `exclude_paths` に登録する、または ②そもそも同期対象フォルダの**外**に置く（別ワークスペース/リポに隔離）。登録忘れが怖いので、機密は最初から同期ツリー外に置くのが最も安全。全体Pushの前は `sync --dry-run` で対象を必ず確認する。
...
| 「My Projectの下に新しいページ作って」 | `new <parent> "Title"` |
...
| pdf | `[📎 PDF名](_assets/file.pdf)` |
| video | `[🎬 Video名](_assets/file.mp4)` |
| audio | `[🔊 Audio名](_assets/file.mp3)` |
| file | `[📁 File名](_assets/file.bin)` |
| bookmark | `[title](url)` |
| embed | `[embed](url)` |
| equation | `$$expression$$` |
| table | パイプ区切りテーブル |
...
- URL は `https://www.notion.so/...` でも `https://app.notion.com/p/<ws>/<slug>-<id>?t=...#anchor`（Copy link 形式・クエリ/アンカー付き）でも可。末尾32桁のページIDを自動抽出（生のID／大文字hexも可）
- pull は front matter の nsync 管理キーのみ更新し、ユーザー定義キー（サブエージェント定義の `name`/`description`/`tools`/`delegable` 等）は温存する（マージ方式。往復で消えない）
...
ローカル画像/PDF等は Notion File Upload API (v2026-03-11) で自動アップロード（≤20MB）。

### 装飾のround-trip（callout / toggle / 色）

Pull時に出力される装飾構文（Notion Enhanced Markdown APIと同一）は、Pushで元のブロック型に完全復元される:
...
- 閉じタグのない `<callout>`/`<details>` は飲み込まれず平文として扱われる（警告表示）
- 不正な `icon`/`color` 属性値は警告付きでドロップされる（push全体は失敗しない）
- 既知の制限: callout内の子ブロックはテキストに平坦化 / toggleの子は100件まで / 本文中に字義通りの `</callout>` 行があると早期終端する

### リンクの扱い（Push時）

| リンク | 挙動 |
|--------|------|
| `[t](https://...)` 等スキーム付き | そのまま送信 |
| `[t](path.md)` 通常の相対パス | そのまま送信（現行APIは受理する） |
| `[t](<path with space.md>)` 山括弧形式 | tree_cacheでタイトル解決できればNotionページリンクに変換。不能なら `t (path)` にテキスト化（pushは失敗しない） |
| `[t](#アンカー)` | テキスト化（Notion APIが拒否するため） |

変換が発生する場合はログに表示される。`push --dry-run` で事前確認可能。
...
```yaml
root_page_id: "12345678-1234-1234-1234-1234567890ab"
label: "My Project"
crawl_max_depth: 10
...
- 推定途中に Notion ページの無い中間フォルダ（`Agents/`・`Documents/` 等）があれば、対応するコンテナページを最寄り既存祖先の配下に自動生成してからその中に作成（既存同名コンテナは再利用・通常運用では重複作成なし。tree_cache が古い場合のみ重複しうる→`refresh`で解消。`--dry-run` では `CREATE container: <名前>` を表示）
- 作成後 `notion_id` をfront matterに自動書き戻し
- 同名ページがNotionに既存 → 警告してスキップ（既存IDにリンク）
- `tree_cache` と `sync_state` も自動更新

**⚠️ push -r の子ページ検出形式:** 子ページとして自動作成されるのは、**単独行の `[📄 タイトル](パス)` 形式**（絵文字アイコン付き）の子リンクのみ。通常のリンク `[タイトル](パス.md)` は本文リンクとして扱われ、子ページは作成されない。この形式の `.md` リンクを検出すると `⚠ NOT A CHILD LINK` 警告が出る（`push --dry-run -r` でも確認可能）。

### Front Matter（新規ページ用）

```yaml
---
notion_parent: 12345678-1234-1234-1234-1234567890ab  # 作成先の親ページID
title: 企画書                                          # ページタイトル
---
```

UUIDは引用符あり（`"..."` / `'...'`）でも可（自動的に除去される）。ただし値の先頭・末尾が**同じ引用符**の場合のみ除去されるため、値自体に引用符を含めたい場合は非対称にするか内側に置くこと。
...
- 4xxエラー（validation等）は即失敗しリトライしない（リトライは429/408/409/5xxのみ）
- `push` は child_page / child_database を保護し、子リンク位置から構造を復元（Position-aware Push）
- `push -r` で新規ページ+子ページの一括作成に対応（子リンクは `[📄 タイトル](パス)` 単独行形式が必須 — New Page Creation節参照）。同名ページがあれば警告してスキップ
...
- Pull時: Notion内部画像/PDF/動画/音声を `_assets/` に自動ダウンロード（S3 URL失効問題を解決）
- Push時: `_assets/` 内のローカルファイルを Notion File Upload API (v2026-03-11) で自動アップロード（≤20MB）
- DB files プロパティ: S3 URLを `_db_assets/` にダウンロードしてローカルパス参照に変換
- round-trip（Pull→Push→Pull）でファイル二重化を防止（元ファイル名ベースのキャッシュ）
- 装飾round-trip: callout/toggle/色はPull→Pushで劣化しない（「装飾のround-trip」節参照。旧形式のquote/リストは従来挙動）
- リネーム検出: Notion側のページ名変更をsync時に検出し、ローカルファイル/フォルダを自動移動
- 単体ファイル自動ページ化: Push時に未参照の画像/PDFを検出し、子ページとして自動作成・Push

## Testing

```bash
python3 -m unittest discover tests    # API・トークン不要のスモークテスト（37件）
```
